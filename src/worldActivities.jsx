...
const kitchenMeasures=[
  {id:'cup',label:'1 cup',short:'1',value:1,kind:'cup'},
  {id:'half',label:'1/2 cup',short:'1/2',value:.5,kind:'cup'},
  {id:'third',label:'1/3 cup',short:'1/3',value:1/3,kind:'cup'},
  {id:'quarter',label:'1/4 cup',short:'1/4',value:.25,kind:'cup'},
  {id:'tbsp',label:'1 tbsp',short:'1 Tbsp',value:1/16,kind:'spoon'},
...
function BakeryMeasuringStation({order,onComplete}){
  ...
  return <section className="kitchenMeasureStation">
    ...
    <div className="measureStationBody">
      <div className="measureRack">
        <b>Measuring cups</b>
        <div className="measureTools">{kitchenMeasures.filter(m=>m.kind==='cup').map(m=><button key={m.id} className={selected===m.id?'selected':''} onClick={()=>setSelected(m.id)}><span className={'measureCup '+m.id}><i style={{height:(m.value*72)+'%'}}/></span><strong>{m.short}</strong></button>)}</div>
        <b>Measuring spoons</b>
        <div className="spoonTools">{kitchenMeasures.filter(m=>m.kind==='spoon').map(m=><button key={m.id} className={selected===m.id?'selected':''} onClick={()=>setSelected(m.id)}><span className={'measureSpoon '+m.id}><i/></span><strong>{m.short}</strong></button>)}</div>
      </div>
      ...
      <div className="mixingStation">
        <div className="mixingBowl"><div className={'bowlFill '+ingredient} style={{height:fill+'%'}}/><span>🥣</span></div>
        <div className="bowlReadout"><small>IN THE BOWL</small><strong>{amountLabel(total)}</strong><span>Goal: {amountLabel(target)}</span></div>
